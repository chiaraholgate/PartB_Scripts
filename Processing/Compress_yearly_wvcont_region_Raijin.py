...
26/2/19 Script modified to process data on raijin.
...
dir_in = '/g/data/xc0/user/Holgate/QIBT/exp02/'
dir_out = dir_in+'Processed/Yearly/'
domain = 'Australia'
regions = ['Australia','MDB','SWWA','SECV','SECN','CC','LEB','NEC','NWP','PBG','SAG','SWP','TTC']

# Load netcdf to use as mask
dir_mask = r'/home/603/cxh603/PhD/PartB/Masks/'
for region in regions:    
    if region == 'Australia':
        ...
        fh.close()
    if region == 'MDB':
        fh = Dataset(dir_mask+'MurrayDarlingBasin.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close() 
    if region == 'SWWA':
        fh = Dataset(dir_mask+'SouthWestCoast.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close() 
    if region == 'SECV':
        fh = Dataset(dir_mask+'SouthEastCoastVictoria.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()     
    if region == 'SECN':
        fh = Dataset(dir_mask+'SouthEastCoastNSW.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()         
    if region == 'CC':
        fh = Dataset(dir_mask+'CarpentariaCoast.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()   
    if region == 'LEB':
        fh = Dataset(dir_mask+'LakeEyreBasin.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()       
    if region == 'NEC':
        fh = Dataset(dir_mask+'NorthEastCoast.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()           
    if region == 'NWP':
        fh = Dataset(dir_mask+'NorthWesternPlateau.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()      
    if region == 'PBG':
        fh = Dataset(dir_mask+'PilbaraGascoyne.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()        
    if region == 'SAG':
        fh = Dataset(dir_mask+'SouthAustralianGulf.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()       
    if region == 'SWP':
        fh = Dataset(dir_mask+'SouthWesternPlateau.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()       
    if region == 'TTC':
        fh = Dataset(dir_mask+'TanamiTimorSeaCoast.nc', mode='r') 
        wsmask = fh.variables['wsmask'][:]
        fh.close()           
