...
echo $this_month,$next_month,$year,$next_year
# "RAINC": cumulative convective rainfall ; "RAINNC": cumulative non-convective rainfall
# > this month
cdo selname,RAINC,Times,XLAT,XLONG ${dir_in}wrfhrly_d01_${year}-${this_month}-01_00:00:00 ${dir_out}RAINC1.nc 
ncrename -v RAINC,RAIN ${dir_out}RAINC1.nc
cdo selname,RAINNC,Times,XLAT,XLONG ${dir_in}wrfhrly_d01_${year}-${this_month}-01_00:00:00 ${dir_out}RAINNC1.nc 
ncrename -v RAINNC,RAIN ${dir_out}RAINNC1.nc
# > next month
cdo selname,RAINC,Times,XLAT,XLONG ${dir_in}wrfhrly_d01_${next_year}-${next_month}-01_00:00:00 ${dir_out}RAINC2.nc 
ncrename -v RAINC,RAIN ${dir_out}RAINC2.nc
cdo selname,RAINNC,Times,XLAT,XLONG ${dir_in}wrfhrly_d01_${next_year}-${next_month}-01_00:00:00 ${dir_out}RAINNC2.nc 
ncrename -v RAINNC,RAIN ${dir_out}RAINNC2.nc

# Find number of days in this month by reading number of time steps
cdo ntime ${dir_out}RAINC1.nc > ${dir_out}timesteps_in_month << EOF
EOF
...
time1=$(($t + 24*$(($day - 1))))
cdo seltimestep,$time1 ${dir_out}RAINC1.nc ${dir_out}temp_${t}_T1_RAINC.nc 
cdo seltimestep,$time1 ${dir_out}RAINNC1.nc ${dir_out}temp_${t}_T1_RAINNC.nc 
time2=$(($time1 + 3))
if [ $day -lt $days_in_month ]; then
cdo seltimestep,$time2 ${dir_out}RAINC1.nc ${dir_out}temp_${t}_T2_RAINC.nc 
cdo seltimestep,$time2 ${dir_out}RAINNC1.nc ${dir_out}temp_${t}_T2_RAINNC.nc 
else # else take first timestep of next month
cdo seltimestep,1 ${dir_out}RAINC2.nc ${dir_out}temp_${t}_T2_RAINC.nc 
cdo seltimestep,1 ${dir_out}RAINNC2.nc ${dir_out}temp_${t}_T2_RAINNC.nc 
fi
period=$(($period + 1))
...
cdo mergetime ${dir_out}temp_*_P_RAINC.nc ${dir_out}wrfhrly_d01_${year}-${this_month}-${today}_00:00:00_RAINC.nc
cdo mergetime ${dir_out}temp_*_P_RAINNC.nc ${dir_out}wrfhrly_d01_${year}-${this_month}-${today}_00:00:00_RAINNC.nc
rm ${dir_out}temp_*_P*.nc

# Sum convective and non-convective rainfall into one file
cdo -f nc4c add ${dir_out}wrfhrly_d01_${year}-${this_month}-${today}_00:00:00_RAINC.nc ${dir_out}wrfhrly_d01_${year}-${this_month}-${today}_00:00:00_RAINNC.nc ${dir_out}wrfhrly_d01_${year}-${this_month}-${today}_00:00:00_RAIN.nc
ncatted -O -a description,,o,c,"Sum of total cumulus and non-cumulus precipitation" ${dir_out}wrfhrly_d01_${year}-${this_month}-${today}_00:00:00_RAIN.nc
