...
Checked and OK 17/1/19.

- Note that if you pass values to a df as [values], you don't need to specifiy an index. see here: 
...
fh.close()
fh = Dataset(dir_mask+'NARCLiM_land_sea_mask.nc', mode='r') 
wsmask_1 = fh.variables['topo'][4:138,4:209] 
fh.close()
wsmask_2 = np.ma.array(wsmask_1,mask=(wsmask==1))
...
if region == 'Australia':
    ...
    #fh = Dataset(dir_mask+'NARCLiM_land_sea_mask.nc', mode='r') 
    #wsmask = fh.variables['topo'][4:138,4:209] 
    #fh.close()
    #wsmask_2 = np.ma.array(wsmask_1,mask=(wsmask==1))
    #wsmask_outside_land  = np.ma.array(wsmask_2,mask=(wsmask_1==0))
if region == 'MDB':
    ...
    del wsmask_Aus, wsmask_1
if region == 'SWWA':
    fh = Dataset(dir_mask+'Aus_Drainage_Divisions/geofabric/Divisions/netcdf/SouthWestCoast.nc', mode='r') 
    wsmask = fh.variables['wsmask'][:] # match shape to results output
    fh.close()      
    fh = Dataset(dir_mask+'NARCliM_AUS_land_sea_mask.nc', mode='r') 
    wsmask_Aus = fh.variables['wsmask'][4:138,4:209] # match shape to results output
    fh.close()    
    ...
    del wsmask_Aus, wsmask_1
if region == 'SECV':
    fh = Dataset(dir_mask+'Aus_Drainage_Divisions/geofabric/Divisions/netcdf/SouthEastCoastVictoria.nc', mode='r') 
    wsmask = fh.variables['wsmask'][:] # match shape to results output
    fh.close()      
    fh = Dataset(dir_mask+'NARCliM_AUS_land_sea_mask.nc', mode='r') 
    wsmask_Aus = fh.variables['wsmask'][4:138,4:209] # match shape to results output
...
#ofile = dir_out+region+'_1979-2013_RR.nc'    
