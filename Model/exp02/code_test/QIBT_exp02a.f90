...
!%%%%%%% OUTPUT %%%%%%%
! The program outputs a daily 2d grid of water vapour contribution of each 
...
! grid cells where it rained that day, lat & lon are the sizes of the 
! domain in the lat and lon directions. Attributes include location and 
! time and amount of precip in the pixel of interest 
...
!
INTEGER, PARAMETER :: totcells = 10000    !total number of grid cells to calculate for
...
INTEGER :: sday = 1,smon = 2,syear = 1979    !start day for calculations
INTEGER :: edday = 2,edmon = 2,edyear = 1979
INTEGER :: totdays
INTEGER, PARAMETER :: totbtadays = 1   !number of days of data to keep for bta; i.e. how far back in time to calc.
...
!CHARACTER(LEN=50), PARAMETER :: fcelllist_xx = "gridcelllist_xx.txt"   !file containing list of grid points to do qibt on
!CHARACTER(LEN=50), PARAMETER :: fcelllist_yy = "gridcelllist_yy.txt" 
...
INTEGER,ALLOCATABLE,DIMENSION(:,:) :: gridcelllist
...
! Allocate in list of grid points you want to run the model for- read in below
ALLOCATE(gridcelllist(totcells),STAT = status)
...
! ! Change to run model for only those grid cells of interest
! totpts = totcells
...
print *,'L2974',gridcelllist_xx, gridcelllist_yy
...
do dd = 1, totdays
  ...
  ! FILE NAME IS A DERIVATIVE OF DAY MONTH YEAR
    open(unit=15,file=TRIM(<FILE NAME>),IOSTAT=status)
	if (status /= 0) then
	print *,"could not get "//TRIM(diri)//TRIM(<FILE NAME>)
	CYCLE
	endif

	do i = 1,totcells
	read(15,*) gridcelllist(i,:) ! CHECK THIS IS READING IN AS 2D
	end do
	close(15)
	
	totpts = !LENGTH OF VALUES IN THE ALLOCATED ARRAY CALLED GRIDCELLLIST
  !! CHECK THERE IS A FILE FOR THAT DAY, IF NOT, "CYCLE"
  !! OPEN FILE CORRESPONDING TO THAT DAY
  !! SET TOTCELLS BASED ON LENGTH OF FILE. TOTPTS = TOTCELLS
  ...
  call calc_pw(mixcld(:,:,:,datatotsteps-datadaysteps:),pres(:,:,:,datatotsteps-datadaysteps:),surf_pres(:,:,datatotsteps-datadaysteps:),ptop,pw)
  ...
  print *,'L3022',gridcelllist_xx, gridcelllist_yy
  ...
  !$OMP PARALLEL DEFAULT(PRIVATE) COPYIN(daytsteps,totsteps,indatatsteps,datadaysteps,datatotsteps,dim_i,dim_j,dim_k,sday,smon,syear,mon,year,day,dd,totpts,ssdim) SHARED(pw,tpw,u,v,w,pres,temp,surf_pres,evap,precip,mix,mixtot,pbl_lev,lat2d,lon2d,orec,outncid,wvcid,wvc2id,xlocid,ylocid,dayid,opreid,wsmask,gridcelllist)
  !!,daylist)  
  ...
  print *,'L3058',gridcelllist_xx, gridcelllist_yy
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  do xx_omp = 1, totpts
    ...
    xx = gridcelllist(xx_omp,1)
    yy = gridcelllist(xx_omp,2)
    print *,'xx=',xx,'yy=',yy
	...
	print *,'L3087 wsmask(xx,yy)=',wsmask(xx,yy)
	if (wshed) then
	...
	print *,'L3091 sum precip(xx,yy)=',SUM(precip(xx,yy,:))
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	  do mm = 1, par_release(tt)
	    ...
	    !par_pot_temp = lin_interp(pot_temp(xx,yy,par_lev,ttdata:ttdata+1),ttfac)
